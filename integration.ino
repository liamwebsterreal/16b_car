...
float theta_left = 0.2922;
float theta_right = 0.2866;
float beta_left = -34.09;
float beta_right = -37.72;
float v_star = 80.8;
...
float driveStraight_left(float delta) {
    float u = (v_star - f_left*delta + beta_left)/theta_left;
    return u;
}

float driveStraight_right(float delta) {
    float u = (v_star + f_right*delta + beta_right)/theta_right;
    return u;
}
...
int run_times[4] = {7000, 2500, 3000, 2950}; // {DRIVE_FAR, DRIVE_LEFT, DRIVE_CLOSE, DRIVE_RIGHT}

float delta_reference(int i) {
  float delta_new = (v_star/5 * i * CAR_WIDTH)/TURN_RADIUS;
  ...
  if (timer_mode == MODE_LISTEN && re_pointer == SIZE){
    ...
    if (envelope(re, result)) {
      ...
      if(best_dist >= EUCLIDEAN_THRESHOLD) {
        ...
        if(best_index == 0) {
          drive_mode = 0; // car = drive far
          start_drive_mode();
        } else if(best_index == 1) {
          drive_mode = 2; // index = drive close
          start_drive_mode();
        } else if(best_index == 2) {
          drive_mode = 1; // population = drive left
          start_drive_mode();
        } else if(best_index == 3) {
          drive_mode = 3; // additionality = drive right
          start_drive_mode();
        } else {
